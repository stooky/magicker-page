 ...
 *
 * Uses shared browser pool to prevent memory exhaustion
 */
import * as cheerio from 'cheerio';
import { acquirePage, releasePage, releasePageWithError, getStats } from '../browser-pool.js';
...
export async function scrapeWithPlaywright(url, options = {}) {
    ...
    const results = {
        ...
        items: [],
        error: null,
        poolStats: null  // Include pool stats in response
    };
    ...
    try {
        // Acquire page from pool (may wait if pool is full)
        const acquired = await acquirePage(url);
        page = acquired.page;
        ...
        try {
            ...
            console.warn('[PLAYWRIGHT] Page load timeout, continuing with partial content...');
        ...
        // Release page back to pool BEFORE parsing (free up resources faster)
        await releasePage(pageId);
        pageId = null;  // Mark as released

        // Parse with Cheerio (doesn't need the page anymore)
        ...
        // Release page with error tracking
        if (pageId) {
            await releasePageWithError(pageId, error);
        }
