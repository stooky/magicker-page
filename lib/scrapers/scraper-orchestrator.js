...
export async function scrapeWebsite(url) {
    ...
    if (cheerioResults.success && cheerioResults.pagesFound > 1) {
        ...
        // Extract full content for Knowledge Base
        const fullContent = cheerioResults.items
        ...
        return {
            ...
            fullContent: fullContent,    // For Knowledge Base (all pages)
    ...
    if (playwrightResults.success) {
        ...
        // Extract full content for Knowledge Base
        const fullContent = playwrightResults.items
            .map(item => `${item.title || item.url}\n${item.content || ''}`)
            .join('\n\n---\n\n');
