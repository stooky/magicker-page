...
import { extractMeaningfulSnippets, formatSnippetsAsNumberedList, generateKnowledgeBaseFAQ } from './openai-extractor.js';
...
export async function scrapeWebsite(url) {
    ...
    if (cheerioResults.success && cheerioResults.pagesFound > 1) {
        ...
        if (useOpenAI) {
            ...
            // Run snippet extraction and FAQ generation in parallel
            const [openaiResult, faqResult] = await Promise.all([
                extractMeaningfulSnippets(cheerioResults.items, url),
                generateKnowledgeBaseFAQ(cheerioResults.items, url, domain)
            ]);
    ...
    if (playwrightResults.success) {
        ...
        if (useOpenAI) {
            ...
            // Run snippet extraction and FAQ generation in parallel
            const [openaiResult, faqResult] = await Promise.all([
                extractMeaningfulSnippets(playwrightResults.items, url),
                generateKnowledgeBaseFAQ(playwrightResults.items, url, domain)
            ]);
