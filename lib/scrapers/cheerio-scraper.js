...
export async function scrapeWithCheerio(url, options = {}) {
    ...
    try {
        ...
        const queued = new Set([url]); // Track URLs already in queue to avoid duplicates
        ...
        while (toVisit.length > 0 && results.pagesFound < maxPages) {
            ...
            try {
                ...
                // Extract internal links for crawling (with deduplication)
                if (depth < maxDepth) {
                    $('a[href]').each((i, elem) => {
                        try {
                            ...
                            // Only follow same-domain links, skip if already visited OR queued
                            if (linkUrl.hostname === baseUrl.hostname &&
                                !visited.has(absoluteUrl) &&
                                !queued.has(absoluteUrl)) {
                                pageData.links.push(absoluteUrl);
                                queued.add(absoluteUrl); // Mark as queued BEFORE adding
