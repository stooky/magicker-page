...
if ($nodeProcesses) {
    ...
    try {
        ...
        Write-Host "  âœ“ All Node.js processes cleaned up" -ForegroundColor Green
        Start-Sleep -Seconds 2
    } catch {
        Write-Host "  âš  Could not kill some processes (may require admin)" -ForegroundColor Yellow
    }
} else {
...
if ($containers -notmatch "botpress") {
    Write-Host "  âœ— Botpress not running" -ForegroundColor Red
    $needsStart = $true
} else {
    Write-Host "  âœ“ Botpress is running" -ForegroundColor Green
}

if ($containers -notmatch "postgres") {
    Write-Host "  âœ— PostgreSQL not running" -ForegroundColor Red
    $needsStart = $true
} else {
