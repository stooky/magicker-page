...
### Step 13: Initialize Webchat with userData
...
1. **Browser → Botpress Cloud**
```http
POST https://messaging.botpress.cloud/v1/webchat/init
Content-Type: application/json
Authorization: Bearer [client token]

{
  "userId": "web_user_12345xyz",  // Auto-generated by webchat
  "userData": {
    "domain": "gibbonheating.com",
    "website": "https://gibbonheating.com",
    "sessionID": "gxqih9"
...
2. **Botpress Cloud Processes:**
   - Creates/updates user record
   - **Stores userData in `user.tags`** (THIS IS KEY!)
   - Returns user object

3. **Botpress Cloud → Browser Response:**
```json
{
...
**CRITICAL DISCOVERY:**
- ✅ userData goes into **`user.tags`**, NOT `user.userData`
- ✅ Accessible in Execute Code as `user.tags.domain`
- ✅ Persists for entire conversation
- ❌ Cannot be updated after init (can only call `init()` once)

**Console Logs:**
```
✅ userData configured in webchat via init()
Available methods: ['init', 'sendEvent', 'onEvent', 'open', 'close', ...]
```
...
const domain = user.tags.domain;  // ← userData goes into user.tags!
//            NOT user.userData.domain ❌
//            NOT user.domain ❌
```

This was discovered through trial, error, and documentation research. It's the key to making domain-specific KB search work.
